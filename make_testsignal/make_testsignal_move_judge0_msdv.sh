...
# #--------------------------------------------SLTFの算出--------------------------------------------#
 for LR in L R; do
   ...
   for Angle in `seq 0 5 355`; do
     ...
     timeconvo $1/HRTF/HRTF_${Angle}_${LR}.DDB $1/RSTF/cinv_cRSTF_${LR}.DDB $1/SLTF/SLTF_$((Angle*10))_${LR}.DDB
   done
done
# # #-------------------------------------------------------------------------------------------------# 
...
#---------------------------------連続音の作成---------------------------------#
Bar=""
for move_width in 1 2 3 4 5; do
  for move_velocity in 2 4 8 16 32; do
    for end_angle in 0; do  
      clear
      echo "牧下は神。makishita is GOD"
      echo "              move width : $move_width deg."
      echo "              move velocity  : $move_velocity °"
      echo "              end angle  : $end_angle deg."
      echo "牧下は神。makishita is GOD makishita makishita makishita makishita"
