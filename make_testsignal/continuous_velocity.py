...
# フーリエ級数窓
fadein_fil = [(a0-a1*cos(pi/overlap_time*i)+a2*cos(2.0*pi/overlap_time*i)-a3*cos(3.0*pi/overlap_time*i)) for i in range(overlap_time)]
fadeout_fil = [(a0+a1*cos(pi/overlap_time*i)+a2*cos(2.0*pi/overlap_time*i)+a3*cos(3.0*pi/overlap_time*i)) for i in range(overlap_time)]

# 音データの読み込み
with open(in_name, 'rb') as sound_bin: sound = fromstring(sound_bin.read(), dtype=int16)
...
for direction in ['c', 'cc']:
    for LR in ['L', 'R']:
        ...
        for connection_number in range(movement_angle): 
            if direction is 'c': data_angle = connection_number - movement_angle + 1
            elif direction is 'cc': data_angle = movement_angle - connection_number - 1

            # 角度が負のとき360を加算
            if data_angle < 0: data_angle += 360

            # SLTFのオープン
            with open(subject + "/SLTF/SLTF_" + str((data_angle+end_angle)*10%360) + "_" + LR + ".DDB", 'rb') as SLTF_bin:
                SLTF = fromstring(SLTF_bin.read(), dtype=float64)
                angle_list.append((data_angle+end_angle)%360) # 使ったSLTFを最後に表示するためのリストを作成
                
            # 音データと伝達関数の畳込み
            sound_SLTF = convolve(sound[connection_number*(duration_time+overlap_time):duration_time+overlap_time*2+(duration_time+overlap_time)*connection_number+len(SLTF)*3+1], SLTF)
        ...
        out = array(out[overlap_time:]).astype(float64) # float64バイナリに変換
        # DSBファイルに書き出し
        out_file = outdir + "/continuous_v" + str(movement_velocity) + direction + "_" + str(end_angle) + "_" + LR + ".DDB"
