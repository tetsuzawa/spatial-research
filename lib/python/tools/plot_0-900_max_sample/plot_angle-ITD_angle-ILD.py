...
def main():
    ...
    # r = [i * 5 for i in range(start, int(end/5), int(step/5))]
    ...
    if step == 1:
        ...
        # df_ITD = df_ITD[start:int(end / 5):int(step / 5)]
        # df_ILD = df_ILD[start:int(end / 5):int(step / 5)]
        # df_ITD = df_ITD[start:int(end/5):step]
        # df_ILD = df_ILD[start:int(end/5):step]
