...
def direction_confirm():
    ...
    for i in range(7): 
        ...
        subprocess("2chplay " + SCRIPT_DIR + subject + "/TSP/TSP_" + str(i*30) + ".DSB")
...
# シャッフル後の順番の記憶
with open(SCRIPT_DIR+subject+"/ANSWER/random_"+args[1]+"_"+times+".txt", 'w') as random_file:
...
for test_num in range(len(test_sounds)):
    while True:
        ...
        # 方向の確認
        if answer=="222": direction_confirm() 
        elif answer=="000": pass
        
        # 回答の入力
        elif int(answer)%10==0 and int(answer)<=180 and int(answer)>=0:

            if test_num == 0: # 表の各項目の書込み
                with open(SCRIPT_DIR+subject+"/ANSWER/answer_"+args[1]+"_"+times+".csv", 'a') as answer_file:
                    answer_file.write("file_name" + "," + "correct_direction" + "," + "answer_direction" + "," + "data_time" + "\n")

            #with open(SCRIPT_DIR+subject+"/ANSWER/answer_"+subject+"_"+times+".csv", 'a') as answer_file:      #directory changed(2019.6.24)
            with open(SCRIPT_DIR+subject+"/ANSWER/answer_"+args[1]+"_"+times+".csv", 'a') as answer_file:
                answer_file.write(test_sounds[test_num] + "," + str(test_direction) + "," + str(answer) + "," + datetime.now().strftime("%H:%M:%S") + "\n")
                break
