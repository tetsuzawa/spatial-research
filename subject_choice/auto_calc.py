...
def main():
    ...
    test_count = np.zeros(7)
    FBC_count = np.zeros(7)
    FBC_rate = np.zeros(8)
    LE = np.zeros(7)
    LE_ave = np.zeros(8)
    LE_sum = np.zeros(7)
    SSD_sum = np.zeros(7)
    SSD = np.zeros(8)  # 8要素(30°ずつ0-180°までの角度と全角度における平均値)
    ...
    with open(subject_dir+"/ANSWER/answer_"+times+".csv", 'r') as answer_file:
        ...
        for m in range(len(test_sounds)):
            ...
            if FBC_flag == 1:
                LE[num] = abs(abs(180 - int(answer)) -
                              int(test_direction))  # 前後誤知覚の補正
            else:
    ...
    with open(subject_dir+"/ANSWER/calc_"+args[1]+"_"+times+".csv", 'a') as calc_file:
        #for i in range(len(test_sounds)): calc_file.write(test_sounds[i]+"\n")
        calc_file.write("Presented_Direrction,FBC_rate,Average_LE,SSD" + "\n")
        for p3 in range(8):
            if int(p3) <= 6:
                calc_file.write(str(
                    p3*30) + "," + str(FBC_rate[p3]) + "," + str(LE_ave[p3]) + "," + str(SSD[p3]) + "\n")
            else:
                calc_file.write(
                    "Average" + "," + str(FBC_rate[p3]) + "," + str(LE_ave[p3]) + "," + str(SSD[p3]) + "\n")
